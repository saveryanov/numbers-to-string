	...
	if(is_bin_search === undefined || !is_bin_search) {
		...
		var transform = function() {
			...
			if(Array.isArray(numbers)) {	// если numbers массив
				...
				var last_elem = numbers.length - 1;
				
				var interval_start = 0;
				var interval_end = 0;
				
				var first_i = 0;
				var last_i = Math.ceil(last_elem/2);
				
				while(interval_end != last_elem) {
					...
					if((last_i - first_i) == (numbers[last_i] - numbers[first_i])) {	// если разница в индексах равна разнице в значениях, то это интервал
						if((last_i) == last_elem || (numbers[last_i + 1] - numbers[last_i]) > 1) {	// если следующий элемент больше текущего больше чем на 1, то конец
							interval_end = last_i;
							...
							first_i = last_i + 1;
							last_i = last_i + 1 + Math.ceil((last_elem - last_i)/2);	// делаем шаг на половину вправо
						} else {
							first_i = last_i;
							last_i = last_i + Math.ceil((last_elem - last_i)/2);	// делаем шаг на половину вправо							
						}
					} else {
						last_i = first_i - Math.ceil((last_elem - last_i)/2);	// делаем шаг на половину влево
					}
				}
			}
			if(callback === undefined) {	// если указан callback, то вывод через него
